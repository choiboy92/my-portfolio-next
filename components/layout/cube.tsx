...
import { Text, OrbitControls, Outlines } from '@react-three/drei';
...
const CubeFace: React.FC<CubeFaceProps> = ({ 
    ...
    useFrame(() => {
        ...
        // Animate face background on hover
        if (faceRef.current && faceRef.current.material) {
            const material = faceRef.current.material as any;
            // const targetOpacity = isHovered ? 0.1 : 0.05;
            // material.opacity = material.opacity + (targetOpacity - material.opacity) * 0.1;
        }
    });
    ...
    return (
        <group position={position} rotation={rotation}>
            <mesh ref={faceRef} position={[0,0,-0.001]}>
                <planeGeometry args={[cubeSize+0.001, cubeSize+0.001]}/>
                {/* <meshBasicMaterial 
                    color={face.color}
                    transparent 
...
const RotatingCube: React.FC<RotatingCubeProps> = ({ 
    ...
    useFrame(() => {
        ...
        cubeRef.current.rotation.x += autoRotateSpeed * 0.2;
        }
    });
...
const CubeNavigation: React.FC<CubeNavigationProps> = ({
    faces = [
